...
export function ResultsDashboard({ analysis, classData, streamingText, streamingDone }: Props) {
  ...
  // Build StatCards type breakdown from categoryBreakdown
  ...
  // Build chronologically-sorted assignment data for charts
  ...
  // Per-type groups with pre-computed grades
  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline gap-2">
        <h2 className="text-lg font-semibold text-white/80">{subject}</h2>
        <span className="text-sm text-white/35">{finalGrade.toFixed(2)}% · {letterGrade}</span>
      </div>
      ...
      {hasBreakdown && (
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="mb-3 text-xs font-medium uppercase tracking-wider text-white/35">Category Breakdown</p>
          <div className="space-y-2">
            {Object.entries(categoryBreakdown!).map(([type, { avg, weight, count }]) => (
              <div key={type} className="flex items-center gap-3">
                <span className="w-10 rounded bg-white/10 px-1.5 py-0.5 text-center text-xs text-white/50">{type}</span>
        ...
        <div className={`grid gap-3 ${assignmentWeight > 0 ? 'grid-cols-3' : 'grid-cols-2'}`}>
          {assignmentWeight > 0 && chronological.length > 0 && (
            <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-center">
              <p className="text-xs text-white/35">Assignments</p>
              <p className="text-xl font-bold text-white">{assignmentAvg.toFixed(2)}%</p>
              <p className="text-xs text-white/30">{(assignmentWeight * 100).toFixed(0)}% of grade</p>
            </div>
          )}
          {midtermPct !== null && (
            <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-center">
              <p className="text-xs text-white/35">Midterm</p>
              <p className="text-xl font-bold text-white">{midtermPct.toFixed(2)}%</p>
              <p className="text-xs text-white/30">{(midtermWeight * 100).toFixed(0)}% of grade</p>
            </div>
          )}
          {finalPct !== null && (
            <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-center">
              <p className="text-xs text-white/35">Final Exam</p>
              <p className="text-xl font-bold text-white">{finalPct.toFixed(2)}%</p>
              <p className="text-xs text-white/30">{(finalWeight * 100).toFixed(0)}% of grade</p>
            </div>
          )}
        </div>
      )}
      ...
      {mainGrades.length > 1 && (
        <div className="rounded-xl border border-purple-500/20 bg-white/5 p-5">
          <h3 className="mb-3 text-sm font-semibold text-purple-300">Grade History</h3>
          <GradeChart
            grades={mainGrades}
      ...
      {showCombined && (
        <div className="rounded-xl border border-purple-500/15 bg-white/5 p-5">
          <h3 className="mb-3 text-sm font-semibold text-purple-300/80">Tests &amp; Quizzes History</h3>
          <CombinedTypeChart
            aItems={tsItems}
            bItems={qzItems}
            labelA="Tests"
      ...
      {soloTypeEntries.map(([type, items], idx) => (
        <div key={type} className="rounded-xl border border-purple-500/15 bg-white/5 p-5">
          <h3 className="mb-3 text-sm font-semibold text-purple-300/80">
